...
def _is_short_phrase(text: str) -> bool:
    """Определяет, является ли текст короткой фразой, которую не стоит использовать как заголовок"""
    if not text:
        return True
    
    text = text.strip().lower()
    
    # Короткие фразы, которые не подходят для заголовков
    short_phrases = [
        "в кг", "в кг мне нужно", "кг", "вес", "по весу",
    ...
    # Проверяем точное совпадение с короткими фразами
    if text in short_phrases:
        return True
    
    # Проверяем, содержит ли текст короткие фразы
    for phrase in short_phrases:
        if phrase in text and len(text) < 20:
            return True
    
    # Если текст короче 10 символов - считаем коротким
    if len(text) < 10:
        return True
...
def _generate_contextual_title(rows: List[Dict[str, Any]], user_text: str) -> str:
    """Генерирует контекстный заголовок на основе данных и текста пользователя"""
    if not rows:
        return "<b>Отчёт</b>"
    
    # Анализируем данные для понимания типа отчета
    first_row = rows[0]
    keys = list(first_row.keys())
    
    # Определяем тип метрики
    metric_type = "Данные"
    if any("weight" in k.lower() or "кг" in k.lower() for k in keys):
        metric_type = "Вес"
    elif any("revenue" in k.lower() or "выручк" in k.lower() or "₽" in k.lower() for k in keys):
        metric_type = "Выручка"
    elif any("quantity" in k.lower() or "колич" in k.lower() for k in keys):
        metric_type = "Количество"
    
    # Определяем группировку
    grouping = "по категориям"
    if any("brand" in k.lower() or "бренд" in k.lower() for k in keys):
        grouping = "по брендам"
    elif any("region" in k.lower() or "регион" in k.lower() for k in keys):
        grouping = "по регионам"
    elif any("manager" in k.lower() or "менеджер" in k.lower() for k in keys):
        grouping = "по менеджерам"
    elif any("channel" in k.lower() or "канал" in k.lower() for k in keys):
        grouping = "по каналам"
    
    # Определяем период из текста пользователя
    period = ""
    user_lower = user_text.lower()
    if "февраль" in user_lower:
        period = " за февраль 2025"
    elif "январь" in user_lower:
        period = " за январь 2025"
    elif "март" in user_lower:
        period = " за март 2025"
    elif "прошлый месяц" in user_lower:
        period = " за прошлый месяц"
    elif "текущий месяц" in user_lower:
        period = " за текущий месяц"
    
    # Формируем заголовок
    title = f"{metric_type} {grouping}{period}"
    return f"<b>{title}</b>"


def _format_text_response(text: str) -> str:
    """Форматирует текстовые ответы для читабельности"""
    if not text:
        return text
    
    logger.info(f"🔧 Форматирование текста: {text[:100]}...")
    
    # Убираем лишние пробелы и переносы
    text = re.sub(r'\s+', ' ', text.strip())
    
    # Разбиваем на предложения и добавляем переносы строк
    ...
    for sentence in sentences:
        ...
        if sentence:
            # Добавляем перенос строки для каждого предложения
            formatted_sentences.append(sentence)
    
    # Соединяем предложения с переносами строк
    ...
    formatted_text = f"<b>Информация</b>\n\n{formatted_text}"
...
def _parse_marked_output(raw: Optional[str]) -> str:
    """Парсер маркированного ответа модели.
    - Заменяет кастомный маркер </n> на перевод строки \n
    - Гарантирует перенос после закрывающего тега </b>
    - Если 'Период:' оказался на той же строке с заголовком — переносит на следующую
    - Принудительно форматирует месяцы и категории в <b> теги
    """
    s = (raw or "").strip()
    if not s:
        return ""
    try:
        # 1) Кастомные маркеры переводов строк
        s = s.replace("</n>", "\n").replace("<n/>", "\n").replace("<br>", "\n").replace("<br/>", "\n")
        
        # 2) Гарантируем перенос сразу после </b> первого заголовка
        s = re.sub(r"</b>\s*", "</b>\n", s, count=1)
        
        # 3) Если 'Период:' примкнул к заголовку — вынесем на новую строку
        s = re.sub(r"</b>\n\s*Период:\s*", "</b>\nПериод: ", s, count=1)
        
        # 4) Принудительно форматируем месяцы в <b> теги (если их нет)
        month_patterns = [
            r'(\d{4}-\d{2})\s*—\s*',  # 2025-01 —
            r'(\d{4}-\d{2})\s*-\s*',   # 2025-01 -
            r'(\d{4}-\d{2})\s*:\s*',   # 2025-01 :
        ]
        for pattern in month_patterns:
            s = re.sub(pattern, r'<b>\1</b> — ', s)
        
        # 5) Форматируем названия месяцев на русском
        russian_months = ['Январь', 'Февраль', 'Март', 'Апрель', 'Май', 'Июнь', 
                         'Июль', 'Август', 'Сентябрь', 'Октябрь', 'Ноябрь', 'Декабрь']
        for month in russian_months:
            # Ищем месяц в начале строки или после переноса, но не в <b> теге
            s = re.sub(rf'(?<!<b>)(?<=\n|^)\s*({month})\s*—', rf'<b>\1</b> —', s)
        
        # 6) Удалим лишние тройные переводы
        s = re.sub(r"\n{3,}", "\n\n", s)
        
        # 7) Если ответ не содержит HTML-тегов, принудительно добавляем базовое форматирование
        if not re.search(r'<[^>]+>', s):
            lines = s.split('\n')
            if lines:
                # Первая строка как заголовок
                lines[0] = f"<b>{lines[0]}</b>"
                # Остальные строки с переносами
                s = '\n\n'.join(lines)
...
SYSTEM_PROMPT = """Ты — интеллектуальный, точный и надёжный агент-аналитик в Telegram-боте. Работаешь с PostgreSQL-базой данных milk (версия 16.9), схема public.
...
{
  "output": "HTML-разметка с маркерами",
  "send_excel": true/false,
  "table_data": [ { ... }, ... ] | null,
  "sql_query": "SELECT ..."
}

HTML-формат (ОБЯЗАТЕЛЬНО):
- ВСЕГДА используй HTML-теги: <b>, <strong>, <i>, <em>, <u>, <ins>, <s>, <del>, <a href="...">, <code>, <pre>.
- Для явных переносов строк используй маркер </n> (ровно так). Мы заменим его на перевод строки.
- Первая строка — заголовок: <b>Краткая формулировка отчёта</b></n>
- Вторая строка (если вычислён период): Период: YYYY-MM-DD — YYYY-MM-DD</n>
- Далее — список строк по одной на категорию (или единственная строка при агрегате), формат:
  <b>Название</b> — 12 345 678,90 ₽</n>
  <b>Название</b> — Метрика1: 12 345,67 ₽; Метрика2: −1 234,56 ₽; Метрика3: 789,00 кг</n>
...
- Если запрос не предполагает БД — всё равно используй HTML-разметку с </n> для переносов.
- ВАЖНО: ВСЕГДА используй <b> для названий месяцев, категорий, брендов и других ключевых элементов!
"""
...
async def run_ai_for_text(*, chat_id: int, user_id: Optional[int], user_name: Optional[str], text: str) -> AgentResult:
    ...
    if sql_query:
        ...
        try:
            ...
            if _valid_html(ai_formatted_output):
                ...
                # Улучшенная логика заголовка: не используем короткие фразы пользователя
                if not existing_title:
                    safe_title = (text or "").strip().rstrip(':')
                    # Проверяем, что заголовок достаточно информативен и не является короткой фразой
                    if safe_title and len(safe_title) > 10 and not _is_short_phrase(safe_title):
                        existing_title = f"<b>{safe_title[0].upper() + safe_title[1:]}</b>"
                    else:
                        # Формируем заголовок на основе данных и контекста
                        existing_title = _generate_contextual_title(rows, text)
                
                # Принудительно форматируем данные из БД в красивый HTML
                if rows and len(rows) > 0:
                    # Определяем период из данных
                    period_line = ""
                    if rows and 'month_start' in rows[0]:
                        dates = [row['month_start'] for row in rows if row.get('month_start')]
                        if dates:
                            min_date = min(dates)
                            max_date = max(dates)
                            period_line = f"Период: {min_date.strftime('%Y-%m-%d')} — {max_date.strftime('%Y-%m-%d')}"
                    
                    # Формируем HTML-отчет
                    html_lines = [existing_title]
                    if period_line:
                        html_lines.append(period_line)
                    
                    # Добавляем данные с форматированием
                    for row in rows:
                        if 'month_start' in row and 'revenue' in row:
                            ...
                            if isinstance(revenue, Decimal):
                                revenue_str = f"{revenue:,.2f}".replace(',', ' ').replace('.', ',')
                            else:
                            ...
                            if percent is not None:
                                if isinstance(percent, Decimal):
                                    percent_str = f"{percent:,.2f}".replace(',', ' ').replace('.', ',')
                                else:
                                    percent_str = str(percent)
                                line = f"<b>{month_name}</b> — Выручка: {revenue_str} ₽; Изменение: {percent_str}%"
                            else:
                                line = f"<b>{month_name}</b> — Выручка: {revenue_str} ₽"
                            
                            html_lines.append(line)
                    
                    output = "\n".join(html_lines)
                    logger.info("✅ Принудительное форматирование данных из БД применено")
                else:
        ...
        # Форматируем текстовые ответы для читабельности
        if output:
            logger.info(f"🔍 Форматирование ответа: {output[:100]}...")
            
            # Принудительно форматируем все текстовые ответы
            if not output.strip().startswith("<b>Продажи") and not output.strip().startswith("<b>Вес") and not output.strip().startswith("<b>Количество"):
                # Это не аналитический отчет - форматируем как информацию
                if not re.search(r'<[^>]+>', output):
                    # Простой текст без HTML
                    output = _format_text_response(output)
                    logger.info("✅ Применено форматирование для простого текста")
                elif not output.strip().startswith("<b>"):
                    # Есть HTML, но нет заголовка
                    output = f"<b>Информация</b>\n\n{output}"
                    logger.info("✅ Добавлен заголовок для HTML ответа")
                else:
                    # Уже есть заголовок, но проверяем форматирование
                    if not re.search(r'\n\n', output):
                        # Нет двойных переносов - добавляем их
                        output = output.replace('\n', '\n\n')
                        logger.info("✅ Добавлены переносы строк")
    ...
    try:
        if output:
            ...
            # Финальная проверка: если ответ не содержит HTML тегов, форматируем его
            if output and not re.search(r'<[^>]+>', output):
                output = _format_text_response(output)
                logger.info("✅ Финальное форматирование применено")
            
            # Принудительное форматирование для всех ответов без заголовков
            if output and not output.strip().startswith("<b>"):
                if not re.search(r'<[^>]+>', output):
                    # Простой текст
                    output = _format_text_response(output)
                    logger.info("✅ Принудительное форматирование для простого текста")
                else:
                    # HTML без заголовка
                    output = f"<b>Информация</b>\n\n{output}"
                    logger.info("✅ Принудительное добавление заголовка")
            
            # Прогон через парсер маркированного ответа (</n>, переносы после </b>, период)
            output = _parse_marked_output(output)
    except Exception:
        pass
    ...
    # ФИНАЛЬНАЯ ПРОВЕРКА: принудительно форматируем все ответы без заголовков
    if output and not output.strip().startswith("<b>"):
        logger.info(f"🚨 ПРИНУДИТЕЛЬНОЕ ФОРМАТИРОВАНИЕ: {output[:100]}...")
        if not re.search(r'<[^>]+>', output):
            # Простой текст - форматируем
            output = _format_text_response(output)
            logger.info("✅ ПРИНУДИТЕЛЬНОЕ форматирование применено")
        else:
            # HTML без заголовка - добавляем заголовок
            output = f"<b>Информация</b>\n\n{output}"
            logger.info("✅ ПРИНУДИТЕЛЬНОЕ добавление заголовка")
