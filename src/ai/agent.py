...
🎯 ОБЯЗАТЕЛЬНАЯ СТРУКТУРА CHART.JS:
{
...
🔧 ВАЖНЫЕ ДЕТАЛИ:
- ВСЕГДА включай \"options\" с полной структурой
- ВСЕГДА включай \"plugins\" с datalabels, legend, title
- ВСЕГДА включай \"scales\" с настройками осей
- datalabels.display = true для показа подписей на графике
- Используй красивые цвета: [\"#FF6384\", \"#36A2EB\", \"#FFCE56\", \"#4BC0C0\", \"#9966FF\"]
- Форматируй большие числа через formatter (M для миллионов, K для тысяч)
...
- "добавь подписи" → добавь в options.plugins.datalabels с display: true
- "убери подписи" → установи в options.plugins.datalabels display: false
- "убери легенду" → установи в options.plugins.legend display: false
- "покажи легенду" → установи в options.plugins.legend display: true
- "поменяй цвета" → измени backgroundColor и borderColor в datasets
- "сделай больше" → добавь width: 1000, height: 800 в корень
- "сделай меньше" → добавь width: 600, height: 400 в корень
- "измени заголовок" → измени text в options.plugins.title

❗️ ВАЖНО: Все изменения должны сохранять структуру options.plugins.*
...
async def process_chart_edit(chat_id: int, text: str, saved_config: Dict[str, Any]) -> AgentResult:
    ...
    logger.info(f"🔄 Starting chart edit for chat {chat_id}, command: {text}")
    logger.info(f"🔄 Current saved config: {json.dumps(saved_config, indent=2)}")
...
🎯 ПРАВИЛА РЕДАКТИРОВАНИЯ CHART.JS:

1. ПОДПИСИ (datalabels) - САМОЕ ВАЖНОЕ:
   - Команды "добавить/показать/отобразить подписи" → добавь в options.plugins.datalabels
   - Команды "убрать/скрыть/спрятать подписи" → установи display: false
   - Структура ВСЕГДА должна быть: "options": {{"plugins": {{"datalabels": {{"display": true/false}}}}}}
   - Если options отсутствует → создай его
   - Если plugins отсутствует → создай его
   - Если datalabels отсутствует → создай его

2. ЛЕГЕНДА:
   - "убрать/скрыть легенду" → "options": {{"plugins": {{"legend": {{"display": false}}}}}}
   - "показать/добавить легенду" → "options": {{"plugins": {{"legend": {{"display": true}}}}}}
   - "легенда справа" → "options": {{"plugins": {{"legend": {{"position": "right"}}}}}}
   - "легенда снизу" → "options": {{"plugins": {{"legend": {{"position": "bottom"}}}}}}

3. ЗАГОЛОВОК:
   - "изменить заголовок" → "options": {{"plugins": {{"title": {{"display": true, "text": "новый заголовок"}}}}}}

4. ЦВЕТА:
   - "поменять цвета" → измени backgroundColor и borderColor в datasets
   - Используй красивые цвета: ["#FF6384", "#36A2EB", "#FFCE56", "#4BC0C0", "#9966FF"]

5. РАЗМЕР:
   - "увеличить/больше" → добавь в корень: "width": 1000, "height": 800
   - "уменьшить/меньше" → добавь в корень: "width": 600, "height": 400
...
7. ОСИ И СЕТКА:
   - "добавить сетку" → "options": {{"scales": {{"x": {{"grid": {{"display": true}}}}}}}}
   - "убрать сетку" → "options": {{"scales": {{"x": {{"grid": {{"display": false}}}}}}}}

📋 СТРУКТУРА ОБЯЗАТЕЛЬНЫХ ПОЛЕЙ:
- ВСЕГДА должен быть "options"
- В options ВСЕГДА должен быть "plugins"
- В plugins должны быть нужные элементы (datalabels, legend, title)
- Сохраняй все существующие настройки
- Добавляй только то, что запрошено

🔧 ПРИМЕР ДОБАВЛЕНИЯ ПОДПИСЕЙ:
Если в конфигурации нет options.plugins.datalabels, то добавь:
"options": {{
  "plugins": {{
    "datalabels": {{
      "display": true,
...
"""
    ...
    try:
        ...
        content = resp.choices[0].message.content if resp.choices else "{}"
        logger.info(f"🔄 AI response for chart edit: {content}")
        
        updated_config = json.loads(content or "{}")
        logger.info(f"🔄 Parsed updated config: {json.dumps(updated_config, indent=2)}")
